document.addEventListener("DOMContentLoaded", function () {
    ...
    .then((data) => {
        ...
        for(let i=0;i<tracks.length;i++){
            totDuration += tracks[i].duration
            
        }

        let minTot = parseFloat(totDuration/60).toFixed(1)
    ...
    <div class="me-5 d-flex flex-column justify-content-md-end">
        ...
        <div  id="dscAlbum" class="d-flex align-items-center">
            ...
            <p class="text-white  mb-0 fw-medium">${data.artist.name} <span class="d-none d-md-inline">• ${data.release_date} • ${data.nb_tracks} brani</span> <span class="text-secondary d-none d-md-inline">${minTot} min</span></p>
    ...
    <p class="text-secondary d-md-none mb-0">Album • ${data.release_date}</p>  `
        ...
        for (let i = 0; i < tracks.length; i++) {
            ...
            trackMin= parseFloat(tracks[i].duration/60).toFixed(2)
            ...
            <div class="col-2 text-end">
                <p class="mb-0 d-none d-md-inline-block">${trackMin}</p>
            </div>
